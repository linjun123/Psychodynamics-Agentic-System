...
class EgoAgent(BaseLLMAgent):
    ...
    def build_payload(self, censor_a_output: CensorAOutput, state: FullInternalState, ego_affect_summary: EgoAffectSummary | None = None) -> dict:
        plan = plan_ego_reality(censor_a_output=censor_a_output, state=state, ego_affect_summary=ego_affect_summary)
        return {
